...
run_extraction() {
    ...
    local date_str=$(date +"%b-%d")
    
    # ALWAYS create Watch/Trash/Failed in TARGET (not source)
    # This ensures proper organization regardless of whether target = source
    local watch="$tgt/Watch_$date_str"
    local trash="$tgt/Trash_$date_str"
    local failed="$tgt/Failed_$date_str"
    
    # If target equals source, create a subfolder structure inside source
    if [ "$tgt" = "$src" ]; then
        watch="$src/Watch_$date_str"
        trash="$src/Trash_$date_str"
        failed="$src/Failed_$date_str"
    fi
